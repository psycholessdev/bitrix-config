...
На странице valkey на DockerHub-е находим нужный нам тег, пример `7.2.9-alpine`.
...
Для версии `8.0.x` настройку выполняем аналогичным образом.
...
- интерпретатор PHP-кода:
  ...
  - берем по умолчанию образ `php:8.2.29-fpm-alpine` / `php:8.3.23-fpm-alpine` / `php:8.4.10-fpm-alpine` и добавляем то, что нам надо через пару слоев сверху
  - собираем `bitrix24/php:8.2.29-fpm-v1-alpine` / `bitrix24/php:8.3.23-fpm-v1-alpine` / `bitrix24/php:8.4.10-fpm-v1-alpine`
- поиск:
...
- сервис для бесплатных SSL-сертификатов от `LetsEncrypt`:
  - используем стабильный образ `goacme/lego:v4.24.0`
  - добавляем логику слоем сверху
  - собираем `bitrix24/lego:4.24.0-v1-alpine`
...
docker pull php:8.3.23-fpm-alpine
docker pull php:8.4.10-fpm-alpine
docker pull node:22
docker pull node:22-alpine
docker pull alpine:3.21
docker pull goacme/lego:v4.24.0
```
...
```bash
cd env-docker/sources/bxphp8323/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/php:8.3.23-fpm-v1-alpine --no-cache .
```

- `bitrix24/php` для версии `8.4.x`:
```bash
cd env-docker/sources/bxphp8410/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/php:8.4.10-fpm-v1-alpine --no-cache .
```
...
- `bitrix24/lego`:
```bash
cd env-docker/sources/bxlego4240/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/lego:4.24.0-v1-alpine --no-cache .
```
